...
const activePlayerTableBody = document.getElementById(
	"active-player-table-body",
);
...
function populateActivePlayerSelect() {
	...
	const currentSel = new Set(
		Array.from(activePlayerSelect.selectedOptions).map((opt) => opt.value),
	);
...
function validateActivePlayerForm() {
	...
	const playtimeOk =
		activePlaytimeInput.value !== "" && Number(activePlaytimeInput.value) > 0;
...
activePlayerSelect.addEventListener("change", () => {
	if (activePlayerField.classList.contains("invalid"))
		validateActivePlayerForm();
});
activeArrivalInput.addEventListener("change", () => {
	if (activeArrivalField.classList.contains("invalid"))
		validateActivePlayerForm();
});
activePlaytimeInput.addEventListener("input", () => {
	if (activePlaytimeField.classList.contains("invalid"))
		validateActivePlayerForm();
});
...
document
	.getElementById("active-player-form")
	.addEventListener("submit", (e) => {
		e.preventDefault();
		if (!validateActivePlayerForm()) return;

		const selectedIds = Array.from(activePlayerSelect.selectedOptions).map(
			(opt) => opt.value,
		);
		...
		[activePlayerField, activeArrivalField, activePlaytimeField].forEach(
			(f) => {
