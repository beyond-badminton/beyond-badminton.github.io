 ...
 *
 * @param {Map<number, Object>} playersMap - Map keyed by player id, each with { name, pick, played, wins, losses, diff, opponents, withdrawn }
...
function rankPlayers(playersMap, inplace = true, tiedOnly = false) {
	...
	flat.forEach((p, i) => {
		p.rank = i + 1;
		if (inplace) {
			const original = playersMap.get(p.id);
			if (original) {
 ...
 *
 * @param {Map<number, Object>} playersMap Map keyed by player id, each an existing player record with an `opponents` map
 ...
 */
function applyMatchScore(playersMap, match, score, sign = 1) {
	...
	match.teamA.forEach((id) => {
		...
		const playerRecord = playersMap.get(id);
	...
	match.teamB.forEach((id) => {
		//console.log(`Processing player ${id} in team B`, players);
	...
	rankPlayers(playersMap);
}

/**
 * @brief Reverts a previously applied match result from playerStats.
 *
 * @param {Map<number, Object>} playersMap keyed by player id, same shape as in applyMatchScore
 * @param {Object} match { teamA: [ids], teamB: [ids] }
